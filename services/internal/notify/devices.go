...
//
// Not scoped by user on purpose: this is how a token Expo reported as
// DeviceNotRegistered gets reaped, and at that point the row may well be
// attached to someone who no longer has the app. It is a system call — a
// request carrying a user must go through DeleteOwnDeviceToken instead.
...
// DeleteOwnDeviceToken detaches a device that belongs to the caller.
//
// Scoped by user because the token arrives in a request body: unscoped, anyone
// with a session could post somebody else's token and silence their phone. The
// row is opaque and hard to guess, which makes that unlikely rather than
// impossible — and "unlikely" is not an authorization check.
//
// A token that is not theirs simply matches nothing, which is also the right
// answer for a sign-out that raced a re-registration on another handset.
func (s *Store) DeleteOwnDeviceToken(ctx context.Context, userID, token string) error {
	_, err := s.pool.Exec(ctx,
		`DELETE FROM device_push_tokens WHERE token = $1 AND user_id = $2::uuid`,
		token, userID)
	return err
}
...
func (h *Handler) unregisterDevice(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	if err := h.store.DeleteOwnDeviceToken(ctx, middleware.UserID(ctx),
		strings.TrimSpace(in.Token)); err != nil {
